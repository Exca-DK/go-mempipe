...
func (shm *SharedMem) Stat() (*Info, error) {
...
func (shma *SharedMemMount) GetOffset() uint {
	return shma.offset
}

// Write places bytes into the shared memory segment.
func (shma *SharedMemMount) SetOffset(v uint) {
	shma.offset = v
}
...
func (shma *SharedMemMount) AtomicWriteUint64(v uint64) error {
	if shma.readonly {
		// see comment on readonly field above
	...
	atomic.StoreUint64((*uint64)((unsafe.Pointer)(uintptr(shma.ptr)+uintptr(shma.offset))), v)
	shma.offset += 8
	return nil
}
...
func (shma *SharedMemMount) AtomicReadUint64() (uint64, error) {
	if (shma.length - shma.offset) < 4 {
		return 0, io.EOF
	}

	v := atomic.LoadUint64((*uint64)((unsafe.Pointer)(uintptr(shma.ptr) + uintptr(shma.offset))))
	shma.offset += 8
	return v, nil
}

func (shma *SharedMemMount) AtomicWriteUint32WithOffset(offset int32, v uint32) error {
	_offset := int32(shma.offset) + offset
	if _offset < 0 || offset > int32(shma.length) {
		return io.EOF
	}

	atomic.StoreUint32((*uint32)((unsafe.Pointer)(uintptr(shma.ptr)+uintptr(_offset))), v)
	shma.offset = uint(_offset)
	return nil
}
...
func (shma *SharedMemMount) AtomicReadUint32WithOffset(offset int32) (uint32, error) {
	_offset := int32(shma.offset) + offset
	if _offset < 0 || offset > int32(shma.length) {
		return 0, io.EOF
	}

	v := atomic.LoadUint32((*uint32)((unsafe.Pointer)(uintptr(shma.ptr) + uintptr(_offset))))
	shma.offset = uint(_offset)
	return v, nil
}
...
func (shma *SharedMemMount) UnreadBytes(amount uint) error {
	if shma.offset == 0 {
		return errors.New("sysvipc: UnreadByte before any ReadByte")
	}
	shma.offset -= amount
